    ...
    <section aria-label="Análisis de indicadores" className="flex h-full min-h-0 flex-col overflow-hidden bg-background font-sans text-foreground">
      ...
      <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
        <aside aria-label="Cuentas y distritos" className="flex max-h-[44%] min-h-0 shrink-0 flex-col border-b bg-card lg:max-h-none lg:w-[400px] lg:border-r lg:border-b-0">
          <div className="p-3"><h3 className="font-heading text-sm font-semibold">Cuentas y distritos</h3><form className="mt-3 flex flex-col gap-1" onSubmit={(event) => { event.preventDefault(); setPage(1); setAppliedSearch(search.trim()) }}><Label className="text-xs font-medium text-muted-foreground" htmlFor="report-search">Buscar por cuenta, cliente o distrito</Label><div className="relative min-w-0 flex-1"><Search className="pointer-events-none absolute top-1/2 left-2.5 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="pl-8" id="report-search" onChange={(event) => setSearch(event.target.value)} placeholder="Ej. cuenta, nombre de cliente o distrito" ref={searchInputRef} value={search} /></div></form></div>
          <Separator />
