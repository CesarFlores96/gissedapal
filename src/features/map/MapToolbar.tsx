...
import { QgisConnectionDialog } from "../gis-import/QgisConnectionDialog"
...
export function MapToolbar(): React.JSX.Element {
  ...
  const { adjustmentMode, clearSelection, searchPlaces, searchSupply, selectPlace } = useSelection()
    ...
    <>
      ...
      <div className="shrink-0" ref={placeWrapperRef}>
        ...
        {placeDropdownOpen && placeAnchorRect ? createPortal(
          <ul
            className="fixed z-50 max-h-72 w-[28rem] space-y-0.5 overflow-y-auto rounded-md border bg-popover p-1 text-popover-foreground shadow-md"
          ...
          >
            {placeResultsOpen ? placeResults.map((result, index) => (
              <li key={`${result.placeId ?? "text"}:${index}`}>
                <Button
                  className="h-auto w-full items-start justify-start gap-2 px-2.5 py-2 text-left"
                  onClick={() => {
                    const view = getViewContext()
                    const near = view ? { lat: (view.bbox[1] + view.bbox[3]) / 2, lng: (view.bbox[0] + view.bbox[2]) / 2 } : undefined
                  ...
                  variant="ghost"
                >
                  <MapPin aria-hidden="true" className="mt-0.5 shrink-0 text-muted-foreground" size={14} strokeWidth={1.75} />
                  <span className="min-w-0 flex-1 whitespace-normal text-sm leading-snug">{result.label}</span>
                </Button>
              </li>
            )) : (
