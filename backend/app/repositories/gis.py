...
async def fetch_supply_detail(pool: AsyncConnectionPool, supply_code: str) -> dict | None:
    return await fetch_one(
        ...
        LEFT JOIN LATERAL (
          SELECT lg.id::text AS record_id, lg.lot_code, b.block_code
          FROM public.gis_cadastral_lot_geometries bridge
          JOIN lot_geometries lg ON lg.id = bridge.gis_lot_id
          LEFT JOIN public.gis_blocks b ON b.id = lg.block_id
          WHERE bridge.cup_code = supply_link.cup_code
            AND (
              logical_lot.gis_lot_match_count = 1
              OR (sl.geom IS NOT NULL AND ST_Covers(lg.effective_geom, sl.geom))
            )
          ORDER BY
            CASE WHEN sl.geom IS NOT NULL AND ST_Covers(lg.effective_geom, sl.geom) THEN 0 ELSE 1 END,
            ST_Area(lg.geom::geography)
          LIMIT 1
        ) structured_lot ON true
        LEFT JOIN LATERAL (
          SELECT lg.id::text AS record_id, lg.lot_code, b.block_code
          FROM lot_geometries lg
          LEFT JOIN public.gis_blocks b ON b.id = lg.block_id
          WHERE structured_lot.record_id IS NULL
            AND sl.geom IS NOT NULL
            AND ST_Covers(lg.effective_geom, sl.geom)
          ORDER BY ST_Area(lg.geom::geography)
          LIMIT 1
        ) spatial_lot ON true
